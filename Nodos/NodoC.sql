...
BEGIN
  ...
  IF (NEW.sucursal IS NOT NULL AND NEW.sucursal <> 3) OR NEW.folio_contrato > max_id THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'No puedes insertar un valor manualmente en folio_contrato y el valor de sucursal debe ser 3 o NULL.';
  END IF;
END;
//
DELIMITER ;
...
-- TRIGGER DE INVERSIÓN PARA IMPEDIR QUE SE META UNA SUCURSAL DIFERENTA LA 3 Y SE INGRESE ALGÚN ID
DELIMITER //
...
DECLARE max_id INT;
  ...
  IF (NEW.sucursal IS NOT NULL AND NEW.sucursal <> 3) OR NEW.folio_inversion > max_id THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'No puedes insertar un valor manualmente en folio_inversion y el valor de sucursal debe ser 3 o NULL.';
  END IF;
END;
//
DELIMITER ;
